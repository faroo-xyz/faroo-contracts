...
- 已观察缺口正常自动F→H，穿透则Catastrophic Insolvency，不改R/P；USDC peg guard只阻断新subscribe，U保持nominal；fast采用固定封顶service fee进F。
当前更新：此前Pharos固定区块节点已验证1153，继续transient latch，不实施理论fallback；真实目标stPROS/SLP版本属于生产集成门槛。APR与LOSS-MATH-01均已关闭，Core Ready见[16](16-insolvency-mode-architecture-freeze.md)；不再要求批准普通storage锁。
