...
4. 16轮更新为F/H自动吸损，穿透则Insolvency；USDC选择peg guard且U nominal；fast选择固定封顶service fee。
当前更新：此前Pharos固定区块节点已验证1153，继续transient latch，不实施理论fallback；真实目标stPROS/SLP版本仍有差异，属于生产集成门槛。APR-01与LOSS-MATH-01均已关闭；Core READY见16，见[14](14-core-architecture-finalization.md)。不再要求批准普通storage锁。
...
| H-06 / deficit全局停付 | F/H吸损，穿透进入objective mode；足额recap恢复 | live R/P index、P-senior、直接删backing、admin clear | bool/id/time；source预算；R/P不改 | syncSolvency/restoreSolvency | E03/INS-01..10 | 16 I01–I16、source/actual-flow stateful | 事故付款时限不保证；真实依赖/生产集成待验 | **CLOSED BY PRODUCT SCOPE REDUCTION / MODEL VERIFIED** |
...
**新风险**：Ucap仍是硬约束；增U前必须按旧U成功实现可实现收益，不能把新资金代入旧elapsed。计划预算校验不等于任意未来价格下保证足额；每次realization必须验对应H，价源/H不足原子失败，不向Reserve临时融资。成熟结算不等待realization，dueAt后不得补旧epoch；部分burn游标及fullburn隔离见14。APR语义已闭合；loss由16的Insolvency替代实时haircut，Core Ready。V1禁止直接stPROS入金，U保持nominal USDC。
...
**Deficit（当前16）**：正常自动吸损F→H；残余穿透用户资产时进入mode，不改R/P。独立sync提交而业务前置拒绝；partial recap不能开领，full recap不恢复F/H。safe登记与普通transfer仍可执行，settle/Claim暂停是已批准catastrophic边界。历史已付资产不clawback，未来事故恢复分发另行设计/审计。旧senior及A/B/C保留负向身份。
...
| E03 Loss / Insolvency | D=max(Q−L,0)，只吸F/H；残余>0入mode；R/P不改 | Claim抢跑、漏sync、partial recap、恶意token回调 | sync提交；业务guard；足额restore；无live index | insolvency_model整数/Fraction quotas、随机stateful | InsolvencyStateful actual-flow/source ghost；生产全量待验 | 16 I01–I16及异常token回滚 | M01/M06/M07 incidentId/entry/current deficit/source余额 |
...
| Claim progress | pure-share累计差额；V1禁止mixed rights | 正常partial/dust加INS mode已通过模型，生产实现另验 |
| Gov trust | ModelA已确认，B不静默选择 | 参数floor/真实角色演练未批准验证 |
| Pause与healthy exit | owner safe解决Guardian准入；不声称token永不冻结 | Insolvency模型已验；真实生产集成/fork尚缺 |
