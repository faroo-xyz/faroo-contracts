...
| --- | --- | --- | --- |
| TbPROSVault | S、R/P/F/H（可追溯base/penalty来源）、U/B/C、epoch/position、计划已资/已计、risk bucket、客观mode/incident元数据 | only authenticated actors；所有资金writer同锁；Gateway交互锁 | Model A Transparent |
...
| syncSolvency() / restoreSolvency() | 实际balance客观吸F/H/入mode或足额恢复，不改R/P；事件保留事故证据 | permissionless，无lossAmount/clear特权；仅balanceOf STATICCALL+本地锁 |
...
删除adminCatchUp及事件、重复redeem/claimAll（独立settlement selector按本轮例外恢复）；分开的checkpointYield、settleMaturedEpochs与显式epoch的claimRedeem。逐入口MustVault/Lens/event/delete/defer清单见[14](14-core-architecture-finalization.md)。Realized Yield与16 Insolvency语义已冻结，Core无Production LossMath字段。设计入口族已明确；生产编译ABI与namespace布局将在单独授权的Skeleton阶段落实，不恢复旧index/units/anchor/carry。

所有正常资金/分类入口共用mode与backing guard，Insolvency下停止；safe/普通share操作/reads/纯治理配置及既有升级路径按[16完整selector矩阵](16-insolvency-mode-architecture-freeze.md)保留。事件新增BuffersAbsorbed、InsolvencyEntered、SolvencyRestored；normal P仍需最小epoch remainingAssets计数用于dust清理，不构成第二提款权。
