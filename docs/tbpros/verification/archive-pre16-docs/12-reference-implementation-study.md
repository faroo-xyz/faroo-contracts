...
| Loss model | F→H→R/P同比已选；多来源H/连续loss仍blocked | 浮动 NAV 与已锁价权利；非本案四桶 waterfall | rate 与实际收到资产；不是四桶 senior waterfall | 赎回债务/策略恢复与 escrow；不可直接映射 |
...
| Loss model | F→H→R/P同比；完整loss模型未冻结 | 信用/资产兑付外部前提 | 流动性/偿还前提 | loss 可先烧 profit buffer，再改 unlock schedule |
...
| Loss model | F→H→R/P同比；连续index待模型 | finalize rate discount，不是 P/F/H waterfall | min rate/maxLoss；大偏差可无法完成 | 只 fail/revert，不解决库存/损失分配 |
...
## RD-10 · Cumulative index / pool loss（本轮增补）
