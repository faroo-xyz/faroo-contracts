...
| H-06 | D/L | 实际token损失/冻结 | 全局停付是政策缺口；tokenloss不一定是Vault bug | F/H吸损，穿透则mode停资金操作；正常P无需恢复率；状态机已测 | 可实现明确分配，不能强迫token付款 | 限额/隔离custody可减损 | 任意tokenfreeze无法纯代码排除 |
...
16轮来源吸损/事故mode/recap模型已测，正常Core无剩余数学blocker；真实目标stPROS/SLP是生产集成依赖，以及生产stateful/gas/storage/deployment/外审。完整V1攻击交易/反证在[归档报告](archive-v1/10-architecture-security-review.md)，当前决策以00/Hard Rules为准，修复状态见10/11。
