...
当前审查入口：[24 · Core Microkernel / Operation Partition](24-core-microkernel-operation-partition-study.md)。**所有已批准V1功能保留。** 9组实编译：M6 Core27,452，最大Manager14,067；业务编排移走后typed context/actual-delta核验抵消节省，Core反增。官方Lens加入完整经济frame fence，Claim已隔离Risk/Token/Oracle/Reserve，但仍依赖复杂YM；YM损坏会阻断Claim/正常sync。**NO — this split still does not solve the protocol；FULL-V1 ARCHITECTURE STILL BLOCKED；PRODUCTION NO-GO。** 23项隔离tests、128×64完整操作stateful及Core91/历史64/Python109/guards通过；恶意Manager反例的PASS只表示攻击复现。生产Solidity/ABI/storage不变，未批准迁移。详见[本地摘要](verification/latest-local-checks.md)。
...
模型成功不能替代生产I01..24/E01..05全量stateful、V1自定义ABI/未支持ID负向测试、完整目标依赖Pharos fork、storage升级兼容、gas/bytecode、ownership handoff与外审。**当前授权止于24轮隔离Microkernel/operation研究，不自动迁移生产、减少V1功能、继续其他业务或采用Category B。**
