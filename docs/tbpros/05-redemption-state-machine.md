...
正常资金操作先mode/backing guard；未同步D>0报SOLVENCY_SYNC_REQUIRED，独立sync只吸F/H，穿透则进入Insolvency。mode中Claim在消费前报INSOLVENT，settlement在burn/锁价/队列推进前拒绝；R/P、U/B及旧num/den不改。没有实时恢复率、loss anchor或position loss carry。

safe继续同queue登记，普通transfer/approve继续。正常月度付款时间保证只适用于足额Solvent运行；catastrophic欠抵押时不保证按月付款。足额实际补资并restore后，成熟节点按原dueAt规则先结算，未实现收益不追补。未来事故分发另行设计/审计，不在V1实现。
