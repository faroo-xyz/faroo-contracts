...
1. 锁定产品决策：ModelA已定；Realized Yield Checkpoint已批准并完成模型验证；V1 custom/share-only、USDC peg guard、F/H吸损与Catastrophic Insolvency、H按来源返还、fixed fee机制已定。LOSS-MATH-01已按产品范围缩减关闭；目标SLP依赖与生产参数属于后续上线门槛。不用fixture默认值填上。
...
从生产快照演练：pending/成熟/已结算/partial share-based Claim、R/P/F/H非零、plan边界、deficit状态、mode entry/partial recap/full restore、所有role/operator、bucket期内使用量；逐字段namespace/array stride与每份债权金额前后比较。语义迁移需要新矩阵，不允许遍历全部历史用户。旧实现可能不能解释H/最终Claim状态/incident mode，不能承诺一键rollback；只在反向兼容已证明时回退，其他用forwardfix。无临时管理员sweep或复制P到第二份账本。
