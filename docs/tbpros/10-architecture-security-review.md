...
用户已选择 **Model A：trusted governance、Transparent 核心**，以及预资stPROS按实际时间释放；最新规则将产品V1改为custom share-based ABI，禁止直接stPROS入金与exact-assets withdraw；已选USDC peg guard、F/H自动吸损与Catastrophic Insolvency、H来源返还、fixed capped fee。APR已MODEL VERIFIED，LOSS-MATH-01按16产品范围缩减关闭，生产参数未批准。
...
**再攻击**：H仅按当前U而不按Ucap预资，则攻击者扩U后计划资金不足；月界若先checkpoint(now)再burn，旧epoch多拿成熟后收益；空池旧H可被新首存捕获。这三条组合仍需实现/测试，H内部同比与退款来源模型已定/已测，旧生产整数loss未闭合属于HISTORICAL NEGATIVE EVIDENCE（[16](16-insolvency-mode-architecture-freeze.md)已移出V1）；H03完整业务集成仍待验证。不得在checkpoint向Reserve临时借资；用户已批准current-price realization；收益和settlement分开，后者无Oracle。E02/I11/I13/I14/I23，RD-05。
...
**当前修改决策（16）**：正常自动F→H，穿透则客观mode，R/P不改。独立permissionless sync提交状态；业务未同步缺口前置SYNC_REQUIRED，进入mode后Claim/settle前置INSOLVENT。部分补资不可解锁，实际足额restore不复活旧F/H。safe登记与普通share转让继续，月度现金时限在事故中不保证。

**再攻击/证据**：R600/P400/F100/H200、L1300→loss301后Alice先Claim零进度拒绝；sync后双方都不可先到先得。余额读取失败/static callback、付款中额外损失、full/partial recap、重复事故、来源守恒及原TL/Gateway升级路径最小回归通过。16将live haircut从V1移除，A/B/C仍失败但不再是Core blocker。事故恢复分发另行设计/审计，不能宣称未知未来loss可追缴历史已付资产。**MODEL VERIFIED / 生产仍UNVERIFIED**。
...
| --- | --- | --- |
| 永久卡资 | 双pause；1raw deficit；上游token全freeze；单节点坏状态使cursor停住 | H01/H06；safe解决准入，真实付款依赖/生产集成仍open；live loss为HISTORICAL NEGATIVE EVIDENCE，当前模式见[16](16-insolvency-mode-architecture-freeze.md) |
| Accounting闭合 | donation不进R；未释放H、P、F不能计入share NAV；fullburn只清R/U/B | 健康L>=R+P+F+H；H终止P变化已修正为0；旧多loss/H实时分配属HISTORICAL NEGATIVE EVIDENCE；[16](16-insolvency-mode-architecture-freeze.md)F/H+mode模型已闭合，生产集成待验 |
...
任意人可以替系统推进settle，但不替用户强制付款；controller/operator保持领取权限。当前V1 catastrophic mode明确暂停付款；safe仅保证准入。正常节点/实际token生产集成未验，不能给全部退出路径liveness通过。旧live loss分支为HISTORICAL NEGATIVE EVIDENCE，见[16](16-insolvency-mode-architecture-freeze.md)。链停机、资产blacklist/冻结/损失、恶意root、用户无gas等无法由本Vault保证固定时间到账；应写明条件，不能用“应急多签会处理”替代未定义的状态转换。
...
| 所有退出路径liveness | safe原语通过；旧loss-aware算法为HISTORICAL NEGATIVE EVIDENCE（[16](16-insolvency-mode-architecture-freeze.md)）；正常真实节点进度/生产依赖未验证 | **FAIL** |
...
**H-06历史负向追加 / High / index归零导致删债或冻结（已移出V1）**：R+P=2^120、g=1e27→连续约90次各半损失→真实raw资产仍>0、整数g却为0。把g=0当全损generation会注销正债权；require(g>0)则后续reconcile可能永久revert。新P的E/g亦可能溢出。修改建议：可达域与有限精度缩放、误差界/零值进度先经独立模型证明；不得治理reset、遍历历史债权或静默将小正损失归零。**再审**：本轮有理数模型不受此下溢，但任意精度有理数不适合生产；Foundry故意复现fail-closed liveness反例，不将它称为修复。当轮LOSS-MATH-01 OPEN；16现已CLOSED BY PRODUCT SCOPE REDUCTION。
...
**Core READY FOR IMPLEMENTATION SKELETON；Production NO-GO。** 原GO条件仍未全部满足：H-01..H-06仍缺各自生产关闭证据；H-07旧语义前提已撤销，APR产品模型已验证。全量资金stateful、真实目标依赖fork、storage compatibility、runtime/gas、handoff、外审均非已验证。本轮不开始完整实现。

2026-09-15最终H-06增量：[16](16-insolvency-mode-architecture-freeze.md)替代live R/P haircut要求。A/B/C1/C2负向证据和旧测试保留；新的mode/recap正常Core状态机通过。没有新恢复token、sweep、admin clear或instant bypass。Core READY不意味着H01..H06的生产集成、fork、storage、gas、handoff和外审已关闭。
