...
'Policy': {'uCap':('USDC raw6','RESERVED legacy global Ucap; fresh init zero, never used for current plan coverage or repurposed'), 'maxMintLossBps':('bps','E01 relative rounding-loss bound; injected, not calibrated'), 'fastFeeBps':('bps','fixed capped service fee'), 'maxFastFeeBps':('bps','hard maximum for service fee in this product version'), 'maxPlanDuration':('seconds','allowed funded-plan duration bound'), 'riskPaused':('bool','blocks subscribe/fast only, no global share pause'), 'requestsPaused':('bool','blocks complex request only; safe ignores')},
...
'Plan': {'fundingUCap':('USDC raw6','frozen per-plan funding term shared by base/penalty; never retroactively changed'), 'id':('counter','monotonic plan identity; identifies source with base/penalty array slot'), 'start':('UTC seconds','inclusive future eligibility start'), 'end':('UTC seconds','exclusive eligibility end'), 'cursor':('UTC seconds','last successful checkpoint clipped to eligible interval; not advanced by partial settlement'), 'status':('enum uint8','Empty/Funded/Active/Retired; retired after full burn prevents old H entering new supply'), 'numeratorRemainder':('USD18 * 10000 * YEAR numerator units','APR division carry <10000*YEAR; not a USD claim; burn scales down, full burn clears'), 'sources':('Source[2]','base then penalty; distributable is derived sum of remaining')},
...
'Proposal': {'expiresAt':('UTC seconds','inclusive execution deadline; no revival of an expired nonce'), 'consumed':('bool','set before upgrade interaction; blocks replay and nested callback execution'), 'nonce':('counter','monotonic proposal/replay identity; persist after consume/cancel'), 'eta':('UTC seconds','queue time + immutable Gateway delay floor'), 'canceled':('bool','cancel marker for current nonce'), 'implementation':('address','exact implementation target for bound proxy only'), 'dataHash':('bytes32','keccak256 migration calldata; never arbitrary executor payload')},
}
rules = {
